...
export class BusComponent implements OnInit {
  ...
  userr:BusbookingDetails=new BusbookingDetails()
  searchu:Bus=new Bus();
  testv:any;
  user:any;
  sourceref:any;
...
bpass:boolean=false
  ...
  test(){
    ...
    this.userr.bus_name=this.fnameref;
  }
  
  busbooking(){    //submit fn
    this.test();
    this.service.busbooking(this.userr).subscribe(data=>{console.log(data)});
    this.pusk=false;
this.disp=false;
this.show=false;
this.bpass=true
...
search(event:any){
 ...
 this.service.searchbus(this.searchu).subscribe(den=>{this.testv=den
   console.log(den);
  ...
  this.fnameref=this.testv[0].bus_code;
  ...
  console.log("busname"+this.fnameref)
...
Showbus(){
  let responce= this.http.get("http://localhost:1010/busses");
